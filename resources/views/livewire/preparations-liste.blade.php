        ...
        <tbody>
            @foreach ($preparations as $preparation)
                <tr class=" hover:bg-gray-200 border-gray-300 cursor-pointer"
                    :class="more == {{ $preparation->id }} ? 'bg-gray-200 border-0 border-t-8 border-gray-50' : ''"
                    ...
                    <td class="px-1 py-2 border text-left " :class="more == {{ $preparation->id }} ? 'font-bold' : ''">
                ...
                <tr class="bg-gray-200" x-show="more == {{ $preparation->id }}" x-transition>
                    <td class="border border-y-0 p-3" colspan="3">
                        <div>
                            <h2 class="font-extrabold text-xl">Description</h2>
                            <p class="px-3">{{ ucfirst($preparation->detail) }}</p>
                        </div>
                        <div class="my-3">
                            <h2 class="font-extrabold text-xl">Composition et matériel</h2>
                            <div class="grid grid-cols-3 gap-3 justify-start">
                        ...
                        <div>
                            <h2 class="font-extrabold text-xl">Préparation</h2>
                ...
                <tr class="bg-gray-200 border-b-8 border-gray-100" x-cloak x-show="more == {{ $preparation->id }}"
                    ...
                    <td class="border border-y-0 p-3 mb-5" colspan="3">
                        @livewire('item-edit-del', ['item' => $preparation], key($preparation->id))
                    </td>
                </tr>
            @endforeach
        </tbody>
