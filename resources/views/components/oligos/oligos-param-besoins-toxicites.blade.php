    ...
    <table class="w-full mt-3">
        ...
        @foreach ($oligovitamines as $oligo => $oligoelement)
            <tr class="border-t-2 border-gray-400">
                <td class="p-3 font-bold border-r-2 border-gray-300">{{ ucfirst($oligoelement) }} </td>
                @foreach ($ateliers as $at => $atelier)
                    <td class="p-3 text-center border-gray-300 border-l-2 bg-vert-100">
                        <input
                            ...
                            type="number" step="1" min="0" value="{{ $besoins[$at][$oligo] }}"
                            wire:model="besoins.{{ $at }}.{{ $oligo }}"
                ...
                @foreach ($especes as $esp => $espece)
                    <td class="p-3 text-center border-gray-300 border-l-2 bg-brique-100">
                        <input
                            ...
                            type="number" step="1" min="0" value="{{ $toxicites[$esp][$oligo] }}"
                            wire:model="toxicites.{{ $esp }}.{{ $oligo }}"
